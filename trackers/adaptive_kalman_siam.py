...
from siamfc.helpers import calculate_average, show_image, read_image, crop_and_resize, initialize_weights, \
    calculate_apce
from siamfc.losses import BalancedLoss
from siamfc.network import Network

__all__ = ['AdaptiveKalmanSiam']


class AdaptiveKalmanSiam(Tracker):
    def __init__(self, weight_path=None):
        super(AdaptiveKalmanSiam, self).__init__('AdaptiveKalmanSiam', True)
        self.config = Config()
    ...
    def update(self, image):
        ...
        if y_prediction < 0:
            y_prediction = 0
        if x_prediction > (image.shape[1] - 1):
            x_prediction = image.shape[1] - 1
        if y_prediction > (image.shape[0] - 1):
            y_prediction = image.shape[0] - 1
        prediction = np.array(
            [
                y_prediction,
                x_prediction
        ...
        window = None
        update = 'prediction'
        if apce_ratio_siam > self.threshold['window']:
            update = 'siamfc'
            ...
            while priority_index <= priority_end:
                ...
                if len(windows) > 0:
                    ...
                    if (window is not None) \
                            ...
                            and (window['apce_ratio'] > self.threshold['window']):
                        update = 'window'
                        break
        ...
        if update == 'siamfc':
            ...
            search_window = np.array(
                [
                    round((prediction[1] + 1) - ((self.x_size * scale_siam) / 2)),
                    round((prediction[0] + 1) - ((self.x_size * scale_siam) / 2)),
            ...
            search_window = np.array(
                [
                    round((window['center'][1] + 1) - ((self.x_size * window['scale']) / 2)),
                    round((window['center'][0] + 1) - ((self.x_size * window['scale']) / 2)),
                    round(self.x_size * window['scale']),
                    round(self.x_size * window['scale'])
                ]
            )
