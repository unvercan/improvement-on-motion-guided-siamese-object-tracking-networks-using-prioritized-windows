...
from tests.core import AlexNetV1, BalancedLoss, Config, Correlation, Net, calculate_average, show_image, read_image, \
    crop_and_resize, init_weights, normalize, calculate_p

__all__ = ['AdaptiveKalmanSiamP']


class AdaptiveKalmanSiamP(Tracker):
    def __init__(self, net_path=None):
        super(AdaptiveKalmanSiamP, self).__init__('AdaptiveKalmanSiamP', True)
        self.config = Config()
    ...
    def update(self, img):
        ...
        window = None
        update = 'prediction'
        if p_ratio_siam > self.threshold['window']:
            update = 'siamfc'
        ...
        # window search
            ...
            while priority_index <= priority_end:
                ...
                if len(windows) > 0:
                    ...
                    if (window is not None) and \
                            not (('p' in window.keys())
                                 and ('p_average' in window.keys())
                                 and ('p_ratio' in window.keys())):
                        # p, p average and p ratio
                        window['p'] = calculate_p(values=window['response'])
                        window['p_average'] = calculate_average(
                            values=(self.p_list + [window['p']]),
                            last_value_count=self.last_value_count
                        )
                        window['p_ratio'] = window['p'] / window['p_average']

                    # box select
                    if (window is not None) \
                            and (window['p_ratio'] > p_ratio_siam) \
                            and (window['p_ratio'] > self.threshold['window']):
                        update = 'window'
                        break
            ...
            # box select
            if ((window is not None)
                and ((p_ratio_siam > window['p_ratio'])
                     and (p_ratio_siam > self.threshold['prediction']))) \
                    or ((window is None)
                        and (p_ratio_siam > self.threshold['prediction'])):
                update = 'siamfc'
        ...
        if update == 'siamfc':
            ...
            # p and p ratio
            if p_ratio_siam > self.threshold['prediction']:
            ...
            # p and p ratio
            if window['p_ratio'] > self.threshold['prediction']:
                self.p_list.append(window['p'])
                self.p_ratio_list.append(window['p_ratio'])
